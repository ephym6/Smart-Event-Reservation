...
<input id="guests" name="guests" type="number" min="1" step="1" inputmode="numeric" value="{{ old('guests') }}" max="{{ (int)($venue->capacity ?? 0) ?: null }}" data-capacity="{{ (int)($venue->capacity ?? 0) }}"
                                   class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" placeholder="Max {{ (int)($venue->capacity ?? 0) ?: '—' }}" />
                        <p id="guests-error" class="mt-1 text-xs text-red-600 hidden">Exceeds capacity (max {{ (int)($venue->capacity ?? 0) ?: '—' }})</p>
                        </div>
                    </div>
...
const estCost = document.getElementById('est-cost');
            ...
            const cap = parseInt("{{ (int)($venue->capacity ?? 0) }}") || 0;
